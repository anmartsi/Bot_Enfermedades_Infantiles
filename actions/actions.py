...
class ActionProvideDiagnosis(Action):
    ...
    def diagnose(self, symptoms, body, severity):
        ...
        if "fiebre" in symptoms or "sarpullido" in symptoms or "dolor de garganta" in symptoms or "erupción" in symptoms or "granitos" in symptoms or "llagas en la boca" in symptoms or "heridas" in symptoms or "ampollas" in symptoms or "boca" in body or "pies" in body or "dolor al tragar" in symptoms or "manos" in body or "lengua" in body:
            return "Si tu hijo presenta llagas en la boca o un sarpullido alrededor de la boca, en las manos o pies, podría tratarse de la enfermedad mano-pie-boca. Si los síntomas no empeoran, no es necesaria una visita al médico. Lo más importante es mantener al niño hidratado hasta su recuperación."
            ...
            return "Si tu hijo tiene la lengua roja, blanca o inflamada, es posible que se haya contagiado de la fiebre escarlatina. Se recomienda acudir al médico para obtener un tratamiento antibiótico acorde."
        elif "fiebre" in symptoms or "sarpullido" in symptoms or "moqueo" in symptoms or "piel enrojecida" in symptoms or "cara" in body or "roja" in symptoms or "mejillas" in symptoms or "malestar" in symptoms or "general" in body or "manchas rojas" in symptoms or "articular" in body or "sarpullido" in symptoms or "cuerpo" in body or "como un tomate" in symptoms or "febrícula" in symptoms or "cachetes" in body or "rojos" in symptoms or "enrojecida" in symptoms or "dolor" in symptoms or "articulaciones" in body:
            return "Si tu hijo tiene un sarpullido amplio en diferentes zonas del cuerpo, pero especialmente en los cachetes, lo más probable es que se trate de un eritema infeccioso o enfermedad de la bofetada. No es necesaria una visita al médico. MUY IMPORTANTE: Evitar el contacto con gestantes, ya que podría ser perjudicial para el feto."
            ...
            return "Parece una gastroenteritis común. Acudir al médico si resulta imposible mantener al niño hidratado."
            ...
            return "Si tu hijo no está vacunado contra la tosferina, parece ser el caso. Acudir al médico si hay vómitos y el niño tiene dificultad para respirar debido a los ataques de tos."
            ...
            return "Si tu hijo tiene una erupción de pequeñas costras que causan gran comezón por todo el cuerpo, lo más seguro es que se trate de varicela. No es necesario acudir al médico si los síntomas no empeoran. Existen a disposición tratamientos para aliviar el picor."
            ...
            return "Si la erupción se trata más bien de manchas y no costras (como en el caso de la varicela) y tu hijo no ha recibido la vacuna triple vírica, lo más probable es que se trate de un caso de sarampión. No existe tratamiento y no es necesario acudir al médico si los síntomas no empeoran."
            ...
            return "Si tu hijo no ha sido vacunado contra las paperas, es muy posible que este sea el caso. Es necesario acudir al médico para obtener el tratamiento correspondiente."
        else:
